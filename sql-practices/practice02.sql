...
-- Ans) 최고임금: 158220, 최저임금: 38623, 최고임금 - 최저임금: 119597
select 
	max(salary) '최고임금', 
	min(salary) '최저임금', 
...
-- 문제4.
-- 현재('9999-01-01') 이 회사의 평균 연봉은 얼마입니까?
-- Ans) 72012.2359
...
-- Ans) 58, 71
select date_format(now(), '%Y')-date_format(max(birth_date), '%Y') 최저나이, 
	   date_format(now(), '%Y')-date_format(min(birth_date), '%Y') 최고나이
from employees;
